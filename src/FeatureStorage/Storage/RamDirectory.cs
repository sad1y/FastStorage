...
public class RamDirectory : IDirectory
{
    ...
    {
        var res = _children.FirstOrDefault(f => f is RamFile { IsDeleted: false } file && file.Name == name);
        return res as IFile;
    }
    ...
    public IEnumerable<IDirectory> GetDirectories() => _children.Cast<RamDirectory>().Where(f => f is not null);

    public ValueTask<IEnumerable<IDirectory>> GetDirectories(CancellationToken token) => new(GetDirectories());

    public IEnumerable<IFile> GetFiles() => _children.Cast<RamFile>().Where(f => f is not null);
    ...
    {
        var res = _children.FirstOrDefault(f => f is RamDirectory { IsDeleted: false } file && file.Name == name);
